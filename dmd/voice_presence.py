...
class VoicePresence:
    ...
    def __init__(self, token: str, guild_id: int, dm_user_id: int | None, tracker: Any) -> None:
        self._token = token
    ...
    async def start(self) -> None:
        ...
        @self._client.event
        async def on_member_speaking_state_update(member: Any, ssrc: Any, state: Any) -> None:
            try:
                speaking = bool(getattr(state, "value", 0) & 1) if hasattr(state, "value") else bool(state)
    ...
    async def _follow_dm(self) -> None:
        ...
        if self._voice and getattr(self._voice, "channel", None) and self._voice.channel.id == dm_channel.id and self._voice.is_connected():
            return
